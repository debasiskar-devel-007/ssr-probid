...
export class TrainingsectionComponent implements OnInit {
  ...
  constructor(es: FormBuilder,public ApiService: ApiService,public _cookieservice:CookieService,public _http:HttpClient,public router:Router,public _router:ActivatedRoute)
  {
    this.es=es;
  ...
  gettrainingcategory()
  {
    const link = this.ApiService.nodesslurl+'datalist?token='+this._cookieservice.get('jwtToken');
   ...
   /* this.files = [];
    ...
    if (output.type === 'allAddedToQueue') {
      const event:UploadInput = {
        type: 'uploadAll',
        url: this.ApiService.nodesslurl + 'uploads',          // this will be changed
        method: 'POST',
      };
...
// console.log(this.dataForm.value);
    ...
    if (this.dataForm.valid) {
      let link = this.ApiService.nodesslurl + 'addorupdatedata?token='+this._cookieservice.get('jwtToken');
